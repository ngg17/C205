...
def recivedMsg():
    ...
    while True:
        ...
        if('player_type' in message):
            ...
            # Dice with value 6
            canvas2.itemconfigure(dice, text='\u2685')
        elif('wins the game.' in message and winingFunctionCall):
        ...
        if('player1Turn' in message and playerType == 'player1'):
            playerTurn = True
            rollButton = Button(gameWindow, text="Roll Dice", fg='black', font=('Chalkboard SE', 15), bg="grey", command=rollDice, height=5, width=20)
            rollButton.place(x=screen_width/2-80, y=screen_height/2+250)
